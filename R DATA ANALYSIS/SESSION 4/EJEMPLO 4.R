...
set.seed(104)
muestra <- rexp(n = 40, rate = 6) # media = 1/6 aprox 0.1667 
# (media de la población)
tail(as.data.frame(muestra))
# estamos interesados en contrastar las hipótesis H0: mu = 0.1 vs
# H1: mu > 0.1 (contraste de cola superior)
...
# Supongamos que estamos interesados en encontrar la región de
# rechazo (de cola superior) con un nivel de significancia alpha = 0.05,
# debemos encontrar el valor z_{0.05} que satisface P(Z > z_{0.05}) = 0.05.
...
set.seed(174)
muestra <- sample(x = c(1, 0), 
                  size = 45, 
                  replace = TRUE, 
                  prob = c(0.67, 0.33)); 0.67 # media real de la población
tail(as.data.frame(muestra))
# estamos interesados en contrastar las hipótesis
# H0: p = 0.9 vs H1: p < 0.9 (contraste de cola inferior)
...
# Supongamos que estamos interesados en encontrar la
# región de rechazo (de cola inferior) con un nivel de
# significancia alpha = 0.05, debemos encontrar el valor
# z_{0.05} que satisface P(Z < z_{0.05}) = 0.05.
...
# estamos interesados en contrastar las
# hipótesis H0: mu1-mu2 = 0 vs H1: mu1-mu2 diferente de 0
# (contraste de dos colas)
...
# Supongamos que estamos interesados en encontrar la
# región de rechazo (de dos colas) con un nivel de 
# significancia alpha = 0.05, debemos encontrar el valor
...
tail(as.data.frame(muestra))
# estamos interesados en contrastar las
# hipótesis H0: mu = 170 vs H1: mu > 170 (contraste de cola superior)

# El valor observado del estadístico de prueba en este caso está dado por

t0 <- (mean(muestra)-170)/(sd(muestra)/sqrt(15)) 
t0
# que proviene de una distribución t de Student 
# con n-1 = 14 grados de libertad (gl).

# Supongamos que estamos interesados en encontrar 
# la región de rechazo (de cola superior) con un nivel 
# de significancia alpha = 0.05, debemos
# encontrar el valor t_{0.05} que satisface
# $P(T > t_{0.05}) = 0.05$, donde $T$ se distribuye
...
# estamos interesados en contrastar las 
# hipótesis H0: mu1-mu2 = 0 vs H1: mu1-mu2 
# diferente de 0 (contraste de dos colas)
...
# Supongamos que estamos interesados en encontrar 
# la región de rechazo (de dos colas) con un nivel de 
# significancia alpha = 0.05, debemos encontrar el valor
